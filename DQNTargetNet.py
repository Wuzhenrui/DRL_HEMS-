...
class Brain:
    ...
    def updateTargetModel(self):                                       #更新模型
...
UPDATE_TARGET_FREQUENCY = 200               #更新目标频率
...
class Agent:
    ...
    def act(self, s, deter):                                                  #espilon贪心   有e的概率选最大的
        if deter == True:
            return numpy.argmax(self.brain.predictOne(s))
        if random.random() < self.epsilon:
            return random.randint(0, self.actionCnt - 1)                      #生成ab之间的随机数
        return numpy.argmax(self.brain.predictOne(s))

    def observe(self, sample):  # in (s, a, r, s_) format
        self.memory.add(sample)
        if self.steps % UPDATE_TARGET_FREQUENCY == 0:         #先存一些经验
...
class Environment:
    ...
    def run(self, agent,day=None):
        #s = self.env.reset(day0=DAY0,dayn=DAYN,day=day)------------------------------这里改了
        ...
        while True:
            ...
            s_, r, done, info = self.env.step(a)          #新的状态和奖励？
...
print(np.average([list(REWARDS[i])[-1] for i in range(DAY0,DAYN)]))    #平均奖励
with open("REWARDS_DQNTNET.pkl", 'wb') as f:
    pickle.dump(REWARDS, f, pickle.HIGHEST_PROTOCOL)      #将这个reward存起来
