...
DEFAULT_POWER_GENERATED = np.genfromtxt("wind_generation_fortum.csv", delimiter=',', skip_header=0, usecols=[-1]) / 100
                                          #读数据  x，y之间用    逗号隔开  usecols=-1  最后一列数据
DEFAULT_WIND_POWER_COST = 3.2
# Balancing market prices
DEFAULT_DOWN_REG = np.genfromtxt("down_regulation.csv", delimiter=',', skip_header=1, usecols=[-1]) / 10           #卖电的价格
DEFAULT_UP_REG = np.genfromtxt("up_regulation.csv", delimiter=',', skip_header=1, usecols=[-1]) / 10             #买电的价格
...
DEFAULT_BASE_LOAD = np.array(
    [.4, .3,.2,.2,.2,.2,.3,.5,.6,.6,.5,.5,.5,.4,.4,.6,.8,1.4,1.2,.9,.8,.6,.5,.4])                            #基本负载   .4 就是0.4
DEFAULT_MARKET_PRICE = 5.48
DEFAULT_PRICE_TIERS = np.array([-3.0, -1.5, 0.0, 1.5, 3.0])                                      #价格默认层
# Battery characteristics (kwh)         电池
...
class TCL:       #恒温控制负载
    ...
    def control(self, ui=0):
        # control TCL using u with respect to the backup controller        #决定是否动作     P4
        if self.T < self.Tmin:
            self.u = 1
    ...
    """ 
    @property allows us to write "tcl.SoC", and it will     soc   =   state of charge   充电状态
...
class Battery:
    ...
    def __init__(self, capacity, useD, dissipation, rateC, maxDD, chargeE):
        self.capacity = capacity  # full charge battery capacity    满电电量
        self.useD = useD  # useful discharge coefficient
        self.dissipation = dissipation  #    电池耗散系数
        self.rateC = rateC  # charging rate  充电率
        self.maxDD = maxDD  # 电池最大传输功率    输出功率
        self.chargeE = chargeE  # max Energy given to the battery to charge    充电功率
        self.RC = 0  # 剩下的电


    def charge(self, E):                                     #电量      充电
        empty = self.capacity - self.RC
        if empty <= 0:
            return E
        else:
            self.RC += self.rateC * min(E,self.chargeE)                      #充电后的电量
            leftover = self.RC - self.capacity + max(E-self.chargeE,0)       #剩下的没被充满的电量
            self.RC = min(self.capacity, self.RC)
            return max(leftover, 0)                                         #返回还有多少电没被充满

    def supply(self, E):                              #供电
        remaining = self.RC
        self.RC -= min(E, remaining,self.maxDD)          #剩下的电  输出最大功率   E应该是输入的那个功率  想要用的
        self.RC = max(self.RC, 0)                           #防止 剩下的电比0少
        return min(E, remaining,self.maxDD) * self.useD        #能够提供的电   乘了一个效率

    def dissipate(self):                                         #自己消耗的能量
        self.RC = self.RC * math.exp(- self.dissipation)                     # exp（）是    e的多少次方
...
class Battery2:
    ...
    def calculate_Zt(self,price,E):

        self.Zt = -price * E        #E是充放电功率



class p_variable_load:                                             #功率可变负载
    def __init__(self,P_Max,P_Min,gear,patience):                          #gear 档位    ,patience 为不满意系数
        self.P_Max = P_Max      #功率最大最小值
        self.P_Min = P_Min
        self.gear  = gear
    ...
    def get_gear_power(self,n):                       #计算各个档位的功率
        self.n = n                                          #   n 是所处的第几个档位
        self.Pg =0                                         #初始化档位功率
        self.Pm = 0                                           # Pm为此时的功率   不直接用Pg可能是为了方便修改
        x = (self.P_Max-self.P_Min)/(self.gear-1)           #每个档位之间的功率间隔
        self.Pg = self.P_Min + x*(n-1)                     #计算第n档的功率    第一档   就是  P_min  所以n-1
        self.Pm = self.Pg

        return self.Pm

    def dis_satisfy(self,n):                                #计算不满意度   n为不满意系数
        self.d_satisfy = 0
        self.d_satisfy = n*(self.P_Max-self.Pg)**2       # P_Max-Pg 的平方   计算不满意度
        return self.d_satisfy

    def calculate_n(self):
        self.Zm = 0
        self.electric_price = 0                          #-------------------------------------------这里的电价还没导入  不知道在哪找
        self.Zm = -self.electric_price * self.Pm - self.d_satisfy
        return self.Zm


class Time_change_load:                              #时间可变负载是启动时间可变   启动之后默认工作一小时结束
    def __init__(self,T_begin,T_end,T_use,Power):
        self.T_end = T_end
        self.T_begin = T_begin
        self.T_use = T_use                         #一次工作的时间
        self.Tn = 0                                 #设备开始工作的时间
        self.Power = Power                        #设备工作的功率

    def dis_satisfy(self,n):                         #n 是不满意系数
        self.Xn = 0
        self.Xn = n*(self.T_begin-self.Tn)           #不满意度
        return self.Xn


    def calculate(self):
        self.Zn = 0
        self.electrcy_price = 0                     #----------------------------------------------------电价
        self.Zn = -self.electrcy_price*self.use*self.Power - self.Xn
        return self.Zn



class electric_car:
    def __init__(self,E_prf,E_max,E_min,E_now,Qev,Nch,T_arr,T_dep,Power,E_arr):
        self.E_prf = E_prf                       #期望电量
        self.E_max = E_max                        #电动汽车 最大电池电量
        self.E_min = E_min                       #最小电量
        self.E_now = E_now                      #当前电量
        self.Qev = Qev                          #电动汽车电池容量   18kwh
        self.Nch = Nch                            #充电效率                0.9
        self.T_arr = T_arr                       #汽车到达时间
        self.T_dep = T_dep                          #离开时间
        self.Power = Power                         #电动车功率
        self.E_arr = E_arr                        #电动车刚来的是时候剩多少电
        self.E_now = self.E_arr

    def current_E(self):                         #计算当前电量

        self.E_now = self.E_now + (self.Power*self.Nch)/self.Qev
        return self.E_now

    def EVZlt(self):                     #计算不满意度
        self.price = 0                #电价
        self.Zlt = - self.price*self.Power
...
class Load:                      #负载
    ...
    def react(self, price_tier , time_day):
        ...
        response = self.price_sens * (price_tier - 2)      #2应该相当于一个阈值   超过2认为贵 可能推迟执行
        if response != 0 :
            self.dr_load -= self.base_load[time_day] * response      #式子  6.7.8 化简应该
            self.shifted_loads[time_day] = self.base_load[time_day] * response
        for k in list(self.shifted_loads):
            probability_of_execution = -self.shifted_loads[k]*(price_tier - 2) + (time_day-k)/self.patience     #式子9
            if random.random()<=probability_of_execution:
                self.dr_load+=self.shifted_loads[k]
                del self.shifted_loads[k]                                 #   执行完这个负载    del 删除
...
class MicroGridEnv(gym.Env):
    ...
    def _build_state(self):
        ...
        price_grid_sell = (price_grid_sell -
                ...
                / np.std(self.grid.sell_prices[self.day*self.iterations:self.day*self.iterations+self.iterations])               #平均值/标准差
...
if __name__ == '__main__':
    ...
    env = MicroGridEnv()
    env.seedy(1)        #设置随机种子列表    用了seed之后   初试位置会固定   gym里面的函数
