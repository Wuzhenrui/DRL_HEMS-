...
class Brain:
    ...
    lock_queue = threading.Lock()     #    锁      返回一个锁对象   其他人要用的时候会遭到阻塞   要排队

    def __init__(self):
        #self.session = tf.Session()      #创建一个会话    用来存放信息   安装的版本可能不匹配  用下一行
        self.session = tf.compat.v1.Session()
        K.set_session(self.session)
        K.manual_variable_initialization(True)     #手动变量初始化

        self.model = self._build_model()
        self.graph = self._build_graph(self.model)

        self.session.run(tf.global_variables_initializer())    #对输入数据进行计算并返回
        self.default_graph = tf.get_default_graph()       #获取当前默认计算图
        self.rewards = REWARDS.copy()

        # self.default_graph.finalize()  # avoid modifications

    def _build_model(self):       #和DQN 的好像一样
    ...
    def _build_graph(self, model):
        #tf.placeholder(shape=[None, 2], dtype=tf.float32)
        tf.compat.v1.disable_eager_execution()    #----------------------------- 下面也改了一点
...
class Agent:
    ...
    def train(self, s, a, r, s_):
        ...
        self.memory.append((s, a_cats, r, s_))      #在列表最后加一个词
